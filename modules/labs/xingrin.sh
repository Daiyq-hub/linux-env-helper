...
_init_xingrin_data() {
    ...
    sudo bash "${docker_dir}/scripts/init-data.sh" 2>&1 | while read -r line; do
        local status_msg=""
    ...
    if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
        msg_warning "数据初始化脚本执行失败，请稍后手动运行:cd ${docker_dir} && sudo bash ${docker_dir}/scripts/init-data.sh"
    fi
}
