...
start_viper() {
    ...
    local viper_pass
    viper_pass=$(grep -E "command:" "${VIPER_DIR}/docker-compose.yml" | grep -oE '\["[^"]+"\]' | tr -d '[]"' || true)
